...
{
    public class DPT_1x : IObjectWithSite
    {
        ...
        public bool value
        {
            ...
            {
                m_value = value;
                m_pParent.oneBitCommand(m_groupAddress, m_value);
            }
        }

        private bool m_valueSync;
        [Description("Permet la synchronisation de la valeur via un retour d'état externe"), Category("Valeur")]
        public bool valueSync
        {
            set
            {
                m_valueSync = value;
        ...
        internal void Feedback(bool value)
        {
            m_value = value;
            (m_pSite as IPNGNotify).NotifyValue(valueId, m_value);
        }
